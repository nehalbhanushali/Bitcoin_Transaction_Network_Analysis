...
class ProcessData(luigi.Task):
    def requires(self):
        return collect_trades.DownloadTradeData()
    ...
    def run(self):
        ...
        def get_book_df(symbol, limit, convert_timestamps=False):
            ...
            books = books.set_index(u'_id')

            #print('values',books.head())
        ...
        def make_features(symbol, sample, mid_offsets,
            ...
            for p in powers:
                print('books',books)
                print('imbalance',get_power_imbalance(books, 10, p))
        ...
        def make_data(symbol, sample):
            ...
            return data
        data = make_data('btc', int('1000'))
        data.to_csv('bitflix-data.csv')
